...
<script>
	function selChange() {
		...
		location.href="review_board?nowPage=${paging.nowPage}&cntPerPage="+sel;
	}
</script>
<meta charset="UTF-8">
<title>REVIEW</title>
...
<div class="bodyback">
<p align="center" class="maintitle">REVIEW</p>
...
<div id="outter">
	 ...
	 <table class="membertable" >
		...
		<c:choose>
			<c:when test="${size ne 0}">
				...
				<td>
					<c:choose>
					<c:when test="${dto.id ne id}"><a href="reviewsearch?reviewnum=${dto.reviewnum}">${dto.reviewtitle}</a></c:when>
					<c:otherwise><a href="myreview?reviewnum=${dto.reviewnum}">${dto.reviewtitle}</a></c:otherwise>
					</c:choose>
				</td>
				<td>${dto.name }</td>
				<td>${dto.writeDate}</td>
	...
	<div class="paging">		
		<c:if test="${paging.startPage != 1 }">
			<a href="review_board?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">이전</a>
		</c:if>
		<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != paging.nowPage }">
					<a href="review_board?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="review_board?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">다음</a>
		</c:if>
	</div>
	</div>
	<div class="bodyinside">
